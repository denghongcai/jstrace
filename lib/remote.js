...
const debug = require('debug')('jstrace:remote');
...
Remote.prototype.invoke = function(){
  ...
  this.actor.on('cleanup', reply => {
    debug('cleanup');
    if (~this.patterns.indexOf('cleanup')) {
...
Remote.prototype.stdout = function(){
  ...
  stream._transform = (chunk, _, done) => {
    ...
    this.actor.send('stdout', chunk, done);
  };
  return stream;
};
...
Remote.prototype.stderr = function(){
  ...
  stream._transform = (chunk, _, done) => {
    ...
    this.actor.send('stderr', chunk, done);
  };
  return stream;
};
