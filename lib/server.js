...
const debug = require('debug')('jstrace:server');
const actorify = require('actorify');
...
Server.prototype.onconnection = function(sock){
  const actor = this.actor = actorify(sock);
...
Server.prototype.prefix = function(str){
  const pre = [this.hostname, this.title, this.pid].join('/');
  return pre + ' >> ' + str;
};
